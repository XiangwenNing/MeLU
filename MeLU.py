...
class MeLU(torch.nn.Module):
    ...
    def store_parameters(self):
        self.keep_weight = deepcopy(self.model.state_dict())  #keep_weight等于模型的参数
    ...
    def forward(self, support_set_x, support_set_y, query_set_x, num_local_update):
        ...
        self.model.load_state_dict(self.keep_weight)   #一个task结束后，模型重新加载keep_weight（即模型参数）
    ...
    def global_update(self, support_set_xs, support_set_ys, query_set_xs, query_set_ys, num_local_update):
        ...
        self.store_parameters()      #一个batch结束后，模型参数已经更新了（99行），所以keep weight要更新一下（更新至跟模型参数相等）
        return
